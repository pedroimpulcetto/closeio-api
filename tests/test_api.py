...
SAMPLE_LEADS_RESPONSE = {"has_more": False, "data": [SAMPLE_LEAD_RESPONSE]}
...
def test_validation_error(api_client):
    responses.add(
        ...
        body=json.dumps(
            {"errors": [], "field-errors": {"lead": "This field is required."}}
        ),
        status=400,
        content_type="application/json",
    )
...
def test_204_responses(api_client):
    responses.add(
        responses.DELETE, "https://api.close.com/api/v1/pipeline/pipe_1234/", status=204
    )
